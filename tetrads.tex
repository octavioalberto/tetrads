...
\begin{remark}
For $q=at+b$, with $a$ even and $b$ odd, the construction of the proof of the main theorem (cf. \cite[Remark 2.8, p. 556]{HS02}) works whenever
\[
\gcd(a,b+2)>1,\quad\text{and}\quad \gcd(a,2b+1)>1,
\]

Indeed, these conditions force $q+2$ and $2q+1$ to be composite for every $K$, so the two possible Wilson-pair collisions are ruled out. The choice $q=6t+1$ is a minimal case and makes the aforementioned pair divisible by same prime divisor, namely $3$.
\end{remark}
